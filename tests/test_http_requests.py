...
def test_user_agent_contains_pandas_version(requests_mock):
    capture = Capture()
    nva = Plot(id="my-plot", create=False, token="nbt-toktok")
    ...
    assert nva.api_read("/data")
    # should look something like NovemLib/0.4.15 Python/3.10.13 Pandas/2.0.0
    assert re.search(r"NovemLib/\d+\.\d+\.\d+ Python/\d+\.\d+\.\d+ Pandas/\d+\.\d+\.\d+", capture.out)
